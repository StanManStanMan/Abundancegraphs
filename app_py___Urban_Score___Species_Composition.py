...
# ── Taxonomic hierarchy (coarse → fine) ────────────────────────────────────
TAX_HIERARCHY = ["tax_kingdom", "tax_phylum", "tax_class", "tax_order",
...
# ── Taxonomic filter ────────────────────────────────────────────────────────
st.sidebar.markdown("---")
st.sidebar.subheader("🔬 Taxonomic filter")

filter_tax_label = st.sidebar.selectbox(
    "Filter by taxonomic level",
    options=list(TAX_OPTIONS.keys()),
    index=list(TAX_OPTIONS.keys()).index("Kingdom"),
    key="filter_tax_label",
)
...
with st.expander("🔎 Active filters", expanded=False):
    st.markdown(
        ...
        f"- **Excluded sites:** {exclude_controls if exclude_controls else 'None'}"
    )
